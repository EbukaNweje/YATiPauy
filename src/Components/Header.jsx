...
const Header = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(`https://yaticare-back-end.vercel.app/api/user/userdata/${user._id}`);
      ...
      {isHomePage ? (
        <div className='homeHcontent'>
          <div className="header-content">
            <div className='profile' onClick={() => navigate('Profile')} title="Go to Profile">
              <FaUser size={30} color='grey' />
            </div>
            <div className="info">
              <h3>Hello, {userData ? userData.userName : "Loading..."}</h3>
              <button disabled><FaNairaSign /> {userData ? `${userData.accountBalance}.00` : "0.00"}</button>
            </div>
          </div>

          <div className="menu-container">
            <div className="Menu" onClick={() => setMenuOpen(!menuOpen)} title="Toggle Menu">
